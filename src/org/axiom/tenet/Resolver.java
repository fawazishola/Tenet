...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
    ...
    public Void visitClassStmt(Stmt.Class stmt) {
        ...
        if (stmt.superclass != null &&
            ...
            Tenet.error(stmt.superclass.name, "A class can't inherit from itself.");
        }
    ...
    public Void visitReturnStmt(Stmt.Return stmt) {
        if (currentFunction == FunctionType.NONE) {
            Tenet.error(stmt.keyword, "Can't return from top-level code.");
        }
        if (stmt.value != null) {
            if (currentFunction == FunctionType.INITIALIZER) {
                Tenet.error(stmt.keyword, "Can't return a value from an initializer.");
            }
            resolve(stmt.value);
        }
        return null;
    ...
    public Void visitThisExpr(Expr.This expr) {
        if (currentClass == ClassType.NONE) {
            Tenet.error(expr.keyword, "Can't use 'this' outside of a class.");
            return null;
        }
    ...
    public Void visitSuperExpr(Expr.Super expr) {
        if (currentClass == ClassType.NONE) {
            Tenet.error(expr.keyword, "Can't use 'super' outside of a class.");
        } else if (currentClass != ClassType.SUBCLASS) {
            Tenet.error(expr.keyword, "Can't use 'super' in a class with no superclass.");
        }
        resolveLocal(expr, expr.keyword);
        return null;
    }
    ...
    // Game theory: resolve game definition
    @Override
    public Void visitGameStmt(Stmt.Game stmt) {
        declare(stmt.name);
        define(stmt.name);
        // Resolve payoff expressions
        for (java.util.Map<StrategyProfile, Expr> playerPayoffs : stmt.payoffs.values()) {
            for (Expr payoffExpr : playerPayoffs.values()) {
                resolve(payoffExpr);
            }
        }
        return null;
    ...
    // Game theory: resolve solve statement
    @Override
